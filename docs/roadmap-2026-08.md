...
1. **Coverage must upgrade to `10.0.0-preview.43`** before building any outbound feature — and the
   ...
   *(Done: master shipped preview.43; branch `adopt-spark-generic-ui` is on **preview.46** and is
   upgrading to **preview.51** — see [adopt-spark-generic-ui.md](adopt-spark-generic-ui.md) M11.
   Note preview.51 adds a model-hash startup gate, so any future preview bump now also needs a
   committed `App_Data/modelHashes.json`.)*
